...
function IntervalEditor({
  ...
  onSave: (data: { customIntervalDays?: number | null }) => void;
}) {
  const es = useStyles(createEditStyles);
  const cid = cadence.customIntervalDays ?? (cadence as any).custom_interval_days;
  const defaultDays = cid ?? template?.intervalDays ?? "";
  const [value, setValue] = useState(String(defaultDays));

  function handleSave() {
    const parsed = parseInt(value, 10);
    const intervalDays = isNaN(parsed) || parsed < 1 ? null : parsed;
    onSave({ customIntervalDays: intervalDays });
  }
...
function CadenceToggleRow({
  ...
  isExpanded: boolean;
  onPress: () => void;
  onSaveSchedule: (data: { customIntervalDays?: number | null }) => void;
}) {
  const styles = useStyles(createCadenceStyles);
  const { colors } = useTheme();
  const cadenceKey = cadence.cadenceKey ?? (cadence as any).cadence_key;
  const customIntervalDays = cadence.customIntervalDays ?? (cadence as any).custom_interval_days;
  const isActive = cadence.isActive ?? (cadence as any).is_active;
  const nextDueDate = cadence.nextDueDate ?? (cadence as any).next_due_date;
  const title = template?.title ?? cadenceKey;
  const days = customIntervalDays ?? template?.intervalDays;
  const schedule = days ? `Every ${days} days` : null;
  const due = isActive ? formatDueDate(nextDueDate) : null;
...
export function TaskSchedulingCard({
  ...
  onSaveSchedule: (id: string, data: { customIntervalDays?: number | null }) => void;
}) {
  const styles = useStyles(createCadenceStyles);
    ...
    <Card>
      ...
      {cadences.map((c) => (
        <CadenceToggleRow
          key={c.id}
          cadence={c}
          template={templateMap.get(c.cadenceKey ?? (c as any).cadence_key)}
