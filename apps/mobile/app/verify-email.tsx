...
/* ---------- Icon badge (avoids extra nesting in card) ---------- */
...
function ActionButton({ label, onPress }: { label: string; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity style={btn.button} onPress={onPress} activeOpacity={0.8}>
      <Text style={btn.buttonText}>{label}</Text>
    </TouchableOpacity>
  );
}

/* ---------- Card content per status ---------- */
...
function LoadingCard() {
  ...
  return (
    <View style={layout.card}>
      ...
      <Text style={txt.loadingText}>Verifying your email...</Text>
    </View>
  );
}

function SuccessCard({ verifiedEmail, onNavigate }: { verifiedEmail: string; onNavigate: () => void }) {
  const layout = useStyles(createLayoutStyles);
  const txt = useStyles(createTextStyles);
  return (
    <View style={layout.card}>
      <StatusIcon variant="success" />
      <Text style={txt.title}>Email Verified!</Text>
      <Text style={txt.subtitle}>Your email has been successfully verified.</Text>
      {verifiedEmail ? <Text style={txt.email}>{verifiedEmail}</Text> : null}
...
function ErrorCard({ message, onNavigate }: { message: string; onNavigate: () => void }) {
  ...
  return (
    <View style={layout.card}>
      ...
      <Text style={txt.title}>Verification Failed</Text>
...
async function verifyEmail(token: string): Promise<{ email?: string }> {
...
export default function VerifyEmailScreen() {
  ...
  useEffect(() => {
    ...
    verifyEmail(token)
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        setErrorMessage(getErrorMessage(err) || "This link has expired or is invalid.");
        setStatus("error");
      });
