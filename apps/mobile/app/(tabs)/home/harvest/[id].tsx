...
function HarvestDetailsCard({
  ...
  return (
    <Card>
      ...
      <InfoRow label="Date" value={formatDate(harvest.harvestedAt)} />
      {harvest.weightKg != null && (
        <InfoRow label="Weight" value={`${toDisplayWeight(harvest.weightKg)} ${weightLabel}`} />
      )}
      {harvest.moisturePercent != null && (
        <InfoRow label="Moisture" value={`${harvest.moisturePercent}%`} />
      )}
      {harvest.honeyType && <InfoRow label="Type" value={harvest.honeyType} />}
      {harvest.color && <InfoRow label="Color" value={harvest.color} />}
      {harvest.framesHarvested != null && (
        <InfoRow label="Frames" value={String(harvest.framesHarvested)} />
      )}
    </Card>
  );
}
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Pressable
        ...
        onPress={() => router.push(`/home/harvest/edit?id=${id}` as any)}
      >
