...
export default function ApiariesScreen() {
  ...
  function handleInspect(hiveId: string) {
    router.push(`/home/inspection/new?hive_id=${hiveId}` as any);
  }
  ...
  const empty = (
    <EmptyState
      ...
      onAction={() => router.push("/home/apiary/new" as any)}
    />
  );
  ...
  function renderApiary({ item }: { item: Apiary }) {
    ...
    return (
      <ApiaryCard
        apiary={item}
        hiveCount={count}
        onPress={() => router.push(`/home/apiary/${item.id}` as any)}
      />
    );
  }
  ...
  return (
    <View style={s.container}>
      ...
      <HexFab onPress={() => router.push("/home/apiary/new" as any)} />
    </View>
  );
}
