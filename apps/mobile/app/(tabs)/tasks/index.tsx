...
function TaskCard({
  ...
  const isInspection = isHiveInspectionTask(task);
  const content = (
    <>
      <TaskCheckbox
      ...
      {isInspection && <ChevronRight size={18} color={colors.textMuted} style={s.chevron} />}
    </>
  );

  if (tappable) {
    return <Pressable style={[s.card, webPointer]} onPress={onPress}>{content}</Pressable>;
  }
  return <View style={s.card}>{content}</View>;
}

function CadencesLink() {
  const s = useStyles(createCadenceStyles);
...
export default function TasksScreen() {
  ...
  function handleToggle(task: TaskModel) {
    ...
    updateTask.mutate({ id: task.id, data: { completedAt: newCompletedAt } });
  }

  function handlePress(task: TaskModel) {
    if (isHiveInspectionTask(task)) {
      router.push(`/home/inspection/new?hive_id=${task.hiveId}&task_id=${task.id}` as any);
    }
  }
