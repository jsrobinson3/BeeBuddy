...
import { useRefreshSync } from "../../../database/useRefreshSync";
...
function formatSchedule(cadence: TaskCadence, template: CadenceTemplate | undefined): string | null {
  ...
  if (template.category === "recurring") {
    const days = cadence.customIntervalDays ?? template.intervalDays;
    return days ? `Every ${days} days` : null;
  }
  if (template.category === "seasonal") {
    const month = cadence.customSeasonMonth ?? template.seasonMonth;
    const day = cadence.customSeasonDay ?? template.seasonDay;
    if (month) {
...
function ScheduleEditor({
  ...
  const [interval, setInterval] = useState(
    String(cadence.customIntervalDays ?? template?.intervalDays ?? ""),
  );
  const [month, setMonth] = useState(
    String(cadence.customSeasonMonth ?? template?.seasonMonth ?? ""),
  );
  const [day, setDay] = useState(
    String(cadence.customSeasonDay ?? template?.seasonDay ?? "1"),
  );

  function handleSave() {
    if (isRecurring) {
      const val = parseInt(interval, 10);
      onSave({ customIntervalDays: isNaN(val) ? null : val });
    } else {
      const m = parseInt(month, 10);
      const d = parseInt(day, 10);
      onSave({
        customSeasonMonth: isNaN(m) ? null : m,
        customSeasonDay: isNaN(d) ? null : d,
      });
    }
  }
...
function CadenceCard({
  ...
  isExpanded: boolean;
  onPress: () => void;
  onSaveSchedule: (data: { customIntervalDays?: number | null; customSeasonMonth?: number | null; customSeasonDay?: number | null }) => void;
}) {
  const styles = useStyles(createCardStyles);
...
export default function CadencesScreen() {
  ...
  const handleToggle = useCallback(
    (id: string, active: boolean) => {
      updateCadence.mutate({ id, data: { isActive: active } });
    },
    [updateCadence],
  );

  const handleSaveSchedule = useCallback(
    (id: string, data: { customIntervalDays?: number | null; customSeasonMonth?: number | null; customSeasonDay?: number | null }) => {
