...
def send_email_sync(to: str, subject: str, html_body: str) -> None:
    ...
    Exceptions are intentionally not caught here so the calling Celery task
    can decide whether to retry (transient failures) or give up (permanent
    4xx responses such as a misconfigured API key).
    """
    settings = get_settings()
