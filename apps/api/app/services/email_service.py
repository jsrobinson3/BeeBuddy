...
def send_email_sync(to: str, subject: str, html_body: str) -> None:
    ...
    Raises on failure so the caller (the Celery task wrapper) can decide
    whether to retry. Previously this swallowed all exceptions, which
    prevented the task's ``self.retry`` from ever firing.
    """
    settings = get_settings()
